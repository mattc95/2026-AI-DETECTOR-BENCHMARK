...
DEFAULT_ENDPOINT = "https://api.gpthumanizer.ai/api/v1/detect"
HUMAN_DETECTOR_CLASSES = {"human", "light_edited", "lightly_edited"}
DEFAULT_DATASETS = (
...
class DetectorResult:
    ...
    detector_class_path: str | None
    predicted_label: str | None
    probabilities: dict[str, Any]
    ai_possibilities: Any
    response: Any
    error: str
...
def parse_headers(raw_headers: list[str]) -> dict[str, str]:
    ...
    api_key = os.getenv("GPTHUMANIZER_API_KEY")
    if api_key:
...
def extract_probabilities(data: Any) -> dict[str, Any]:
    found = find_key(data, "probabilities")
    if found is None:
        return {}
    value, _path = found
...
def normalize_probability_map(probabilities: dict[str, Any]) -> dict[str, float | None]:
    return {key: normalize_percent(value) for key, value in probabilities.items()}


def detector_class_to_label(detector_class: str | None) -> str | None:
    if detector_class is None:
        return None
    normalized_class = detector_class.strip().lower()
    if not normalized_class:
        return None
    if normalized_class in HUMAN_DETECTOR_CLASSES:
        return "human"
    return "ai"
...
def load_dataset(dataset_name: str, input_path: Path) -> list[EvalJob]:
...
def classify_text(
    ...
    for attempt_number in range(retries + 1):
        try:
            ...
            class_found = find_key(response_value, "class")
            detector_class = None
            detector_class_path = None
            if class_found is not None:
                detector_class_value, detector_class_path = class_found
                detector_class = str(detector_class_value).strip().lower()

            probabilities = extract_probabilities(response_value)
            ai_possibilities_found = find_key(response_value, "ai_possibilities")
            ai_possibilities = ai_possibilities_found[0] if ai_possibilities_found else None

            return DetectorResult(
                status="ok" if detector_class else "missing_class",
                http_status=response.status_code,
                detector_class=detector_class,
                detector_class_path=detector_class_path,
                predicted_label=detector_class_to_label(detector_class),
                probabilities=probabilities,
                ai_possibilities=ai_possibilities,
                response=response_value if include_response else None,
                error="" if detector_class else "Could not find class in response",
            )
    ...
    return DetectorResult(
        ...
        probabilities={},
        ai_possibilities=None,
        response=last_response if include_response else None,
        error=last_error,
    )
...
def build_output_item(job: EvalJob, result: DetectorResult) -> dict[str, Any]:
    probability_percent = normalize_probability_map(result.probabilities)
    ...
    return {
        ...
        "detector_class": result.detector_class,
        "detector_class_path": result.detector_class_path,
        "predicted_label": predicted_label,
        "correct": correct,
        "probabilities": result.probabilities,
        "probability_percent": probability_percent,
        "ai_possibilities": result.ai_possibilities,
        "ai_possibilities_percent": normalize_percent(result.ai_possibilities),
...
def summarize_records(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    detector_class_counts = Counter(str(record.get("detector_class")) for record in records if record.get("detector_class"))
...
def build_metrics(records: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "by_detector_class": grouped_metrics(records, "detector_class"),
    }
...
def save_output(
    ...
    payload = {
        "meta": {
            ...
            "prediction_rule": "detector class human/light_edited/lightly_edited => human; all other classes => ai",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run detector on human/AI test JSON files and evaluate binary metrics.")
    parser.add_argument(
        "--dataset",
        action="append",
        default=[],
    ...
    parser.add_argument(
        "-o",
        "--output",
        default="output/detection_eval_output.json",
        help="Single JSON output file for detector results and metrics.",
    )
    parser.add_argument("--endpoint", default=DEFAULT_ENDPOINT, help="Detector API endpoint")
    ...
    parser.add_argument("--header", action="append", default=[], help="Extra HTTP header, such as 'Authorization=Bearer token'")
    parser.add_argument("--no-response", action="store_true", help="Do not store raw detector response in the output file")
...
def main() -> int:
    ...
    for dataset_name, dataset_path in dataset_specs:
        if not dataset_path.exists():
            ...
            return 1
        dataset_jobs = load_dataset(dataset_name, dataset_path)
    ...
    print("Rule: detector class human/light_edited/lightly_edited => human; all other classes => ai")
    ...
    try:
        for batch_number, batch in enumerate(iter_batches(jobs, args.rate_per_second), start=1):
            ...
            batch_results: dict[str, DetectorResult] = {}
            ...
            if args.save_every and processed_count - saved_count >= args.save_every:
                save_output(output_path, endpoint, dataset_specs, output_items, include_response)
