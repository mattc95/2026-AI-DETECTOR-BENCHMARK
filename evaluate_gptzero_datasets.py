...
class GPTZeroResult:
    ...
    version: str | None
    neat_version: str | None
    document: dict[str, Any] | None
    predicted_class: str | None
    predicted_class_path: str | None
...
def normalize_probability_map(probabilities: Any) -> dict[str, float | None]:
    if not isinstance(probabilities, dict):
        return {}
    return {key: normalize_percent(value) for key, value in probabilities.items()}
...
def first_document(response_value: Any) -> dict[str, Any] | None:
    if not isinstance(response_value, dict):
        return None
    documents = response_value.get("documents")
    if isinstance(documents, list) and documents and isinstance(documents[0], dict):
        return documents[0]
    return None


def extract_predicted_class(response_value: Any) -> tuple[str | None, str | None]:
    document = first_document(response_value)
    if isinstance(document, dict) and document.get("predicted_class") is not None:
        predicted_class = str(document.get("predicted_class") or "").strip().lower()
        return predicted_class or None, "documents.0.predicted_class"

    found = find_key(response_value, "predicted_class")
    if found is None:
        return None, None
    predicted_class = str(found[0] or "").strip().lower()
    return predicted_class or None, found[1]


def predicted_class_to_label(predicted_class: str | None) -> str | None:
    if predicted_class is None:
        return None
    normalized_class = predicted_class.strip().lower()
    if normalized_class == "human":
        return "human"
    if normalized_class in GPTZERO_AI_CLASSES:
        return "ai"
    return None
...
def classify_text(
    ...
    for attempt_number in range(retries + 1):
        try:
            response = requests.post(
                endpoint,
                headers=headers,
                json=build_request_body(job.text, multilingual, model_version, api_version),
                timeout=timeout,
            )
            ...
            document = first_document(response_value)
            predicted_class, predicted_class_path = extract_predicted_class(response_value)
            predicted_label = predicted_class_to_label(predicted_class)
            ...
            if not isinstance(response_value, dict):
                return GPTZeroResult(
                    ...
                    predicted_class_path=predicted_class_path,
            ...
            if document is None:
                return GPTZeroResult(
                    ...
                    http_status=response.status_code,
                    version=str(response_value.get("version") or "") or None,
                    neat_version=str(response_value.get("neatVersion") or "") or None,
                    document=None,
                    predicted_class=predicted_class,
                    predicted_class_path=predicted_class_path,
                    predicted_label=predicted_label,
                    response=response_value,
                    error="Could not find documents[0] in response",
                )
            if predicted_class is None:
                return GPTZeroResult(
                    status="missing_predicted_class",
                    http_status=response.status_code,
                    version=str(response_value.get("version") or "") or None,
                    neat_version=str(response_value.get("neatVersion") or "") or None,
                    document=document,
                    predicted_class=None,
                    predicted_class_path=None,
                    predicted_label=None,
                    response=response_value,
                    error="Could not find predicted_class in response",
                )
            if predicted_label is None:
                return GPTZeroResult(
                    status="unknown_predicted_class",
                    http_status=response.status_code,
                    version=str(response_value.get("version") or "") or None,
                    neat_version=str(response_value.get("neatVersion") or "") or None,
                    document=document,
                    ...
                    predicted_label=None,
                    response=response_value,
                    error=f"Unknown predicted_class: {predicted_class}",
                )
    ...
    predicted_class, predicted_class_path = extract_predicted_class(last_response)
    return GPTZeroResult(
        status="error",
        http_status=last_http_status,
        version=str(last_response.get("version") or "") if isinstance(last_response, dict) else None,
        neat_version=str(last_response.get("neatVersion") or "") if isinstance(last_response, dict) else None,
        document=first_document(last_response),
        predicted_class=predicted_class,
        predicted_class_path=predicted_class_path,
        predicted_label=predicted_class_to_label(predicted_class),
...
def document_value(document: dict[str, Any] | None, key: str) -> Any:
    if isinstance(document, dict):
        return document.get(key)
    return None


def build_output_item(job: EvalJob, result: GPTZeroResult) -> dict[str, Any]:
    predicted_label = result.predicted_label
    correct = predicted_label == job.true_label if predicted_label is not None else None
    class_probabilities = document_value(result.document, "class_probabilities")
    average_generated_prob = document_value(result.document, "average_generated_prob")
    completely_generated_prob = document_value(result.document, "completely_generated_prob")
    return {
        "dataset": job.dataset,
        "id": job.item_id,
        "text": job.text,
        ...
        "neatVersion": result.neat_version,
        "document_version": document_value(result.document, "version"),
        "document_id": document_value(result.document, "document_id"),
        "predicted_class": result.predicted_class,
        "predicted_class_path": result.predicted_class_path,
        "predicted_label": predicted_label,
        "correct": correct,
        "average_generated_prob": average_generated_prob,
        "average_generated_prob_percent": normalize_percent(average_generated_prob),
        "completely_generated_prob": completely_generated_prob,
        "completely_generated_prob_percent": normalize_percent(completely_generated_prob),
        "class_probabilities": class_probabilities,
        "class_probabilities_percent": normalize_probability_map(class_probabilities),
        "confidence_category": document_value(result.document, "confidence_category"),
        "confidence_score": document_value(result.document, "confidence_score"),
        "document_classification": document_value(result.document, "document_classification"),
        "result_message": document_value(result.document, "result_message"),
        "result_sub_message": document_value(result.document, "result_sub_message"),
        "paragraphs": document_value(result.document, "paragraphs"),
        "sentences": document_value(result.document, "sentences"),
        "subclass": document_value(result.document, "subclass"),
        "gptzero_document": result.document,
...
def summarize_records(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    predicted_class_counts = Counter(str(record.get("predicted_class")) for record in records if record.get("predicted_class"))
    confidence_counts = Counter(str(record.get("confidence_category")) for record in records if record.get("confidence_category"))
    document_classification_counts = Counter(
        str(record.get("document_classification")) for record in records if record.get("document_classification")
    )
    ...
    return {
        ...
        "predicted_class_counts": dict(predicted_class_counts),
        "confidence_category_counts": dict(confidence_counts),
        "document_classification_counts": dict(document_classification_counts),
...
def build_metrics(records: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "by_predicted_class": grouped_metrics(records, "predicted_class"),
        "by_confidence_category": grouped_metrics(records, "confidence_category"),
        "by_document_classification": grouped_metrics(records, "document_classification"),
    }
...
def save_output(
    ...
    payload = {
        "meta": {
            ...
            "prediction_rule": "predicted_class human => human; predicted_class ai/mixed => ai",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run GPTZero predict/text on human/AI test JSON files and evaluate metrics.")
    parser.add_argument(
        "--dataset",
        action="append",
        default=[],
    ...
    parser.add_argument(
        "-o",
        "--output",
        default="output/gptzero_detection_eval_output.json",
        help="Single JSON output file for GPTZero results and metrics.",
    )
    parser.add_argument("--field", default="text", help="Input JSON field sent to GPTZero as document")
    parser.add_argument("--endpoint", default=DEFAULT_ENDPOINT, help="GPTZero predict/text API endpoint")
    ...
    parser.add_argument("--header", action="append", default=[], help="Extra HTTP header, such as 'x-api-key=token'")
    parser.add_argument("--multilingual", action="store_true", help="Use GPTZero multilingual detection when supported")
    parser.add_argument("--model-version", default=None, help="Optional GPTZero modelVersion request value")
    parser.add_argument("--api-version", default=None, help="Optional GPTZero apiVersion request value")
...
def main() -> int:
    ...
    print("Request field: document")
    print(f"Items to request: {len(jobs)}, resumed: {resumed_count}")
    print("Rule: predicted_class human => human; predicted_class ai/mixed => ai")
    ...
    try:
        for batch_number, batch in enumerate(iter_batches(jobs, args.rate_per_second), start=1):
            ...
            batch_results: dict[str, GPTZeroResult] = {}
            ...
            for job in batch:
                key = output_key(job)
                output_items[key] = build_output_item(job, batch_results[key])
                processed_count += 1
