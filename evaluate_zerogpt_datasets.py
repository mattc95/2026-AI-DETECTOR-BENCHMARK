...
def response_data(response_value: Any) -> Any:
    if isinstance(response_value, dict):
        return response_value.get("data")
    return None


def extract_feedback(response_value: Any) -> str | None:
    data = response_data(response_value)
    if isinstance(data, dict) and data.get("feedback") is not None:
        feedback = str(data.get("feedback") or "").strip()
        return feedback or None

    found = find_key(response_value, "feedback")
    if found is None:
        return None
    feedback = str(found[0] or "").strip()
    return feedback or None


def feedback_to_label(feedback: str | None) -> str | None:
    if feedback is None:
        return None
    if HUMAN_FEEDBACK_MARKER in feedback.lower():
        return "human"
    return "ai"
...
def classify_text(
    ...
    for attempt_number in range(retries + 1):
        try:
            ...
            feedback = extract_feedback(response_value)
            data = response_data(response_value)
            predicted_label = feedback_to_label(feedback)
            ...
            if feedback is None:
                return ZeroGPTResult(
                    ...
                    http_status=response.status_code,
                    success=response_value.get("success"),
                    ...
                    response=response_value,
                    error="Could not find feedback in response",
                )
    ...
    return ZeroGPTResult(
        ...
        feedback=extract_feedback(last_response),
        predicted_label=feedback_to_label(extract_feedback(last_response)),
...
def data_value(data: Any, key: str) -> Any:
    if isinstance(data, dict):
        return data.get(key)
    return None


def build_output_item(job: EvalJob, result: ZeroGPTResult) -> dict[str, Any]:
    predicted_label = result.predicted_label
    correct = predicted_label == job.true_label if predicted_label is not None else None
    fake_percentage = data_value(result.data, "fakePercentage")
    return {
        "dataset": job.dataset,
        "id": job.item_id,
        "text": job.text,
        ...
        "predicted_label": predicted_label,
        "correct": correct,
        "isHuman": data_value(result.data, "isHuman"),
        "textWords": data_value(result.data, "textWords"),
        "aiWords": data_value(result.data, "aiWords"),
        "fakePercentage": fake_percentage,
        "fakePercentage_percent": normalize_percent(fake_percentage),
        "additional_feedback": data_value(result.data, "additional_feedback"),
        "sentences": data_value(result.data, "sentences"),
        "h": data_value(result.data, "h"),
        "hi": data_value(result.data, "hi"),
        "specialIndexes": data_value(result.data, "specialIndexes"),
        "specialSentences": data_value(result.data, "specialSentences"),
        "originalParagraph": data_value(result.data, "originalParagraph"),
        "response_input_text": data_value(result.data, "input_text"),
        "zerogpt_id": data_value(result.data, "id"),
        "zerogpt_data": result.data,
...
def summarize_records(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    feedback_counts = Counter(str(record.get("feedback")) for record in records if record.get("feedback"))
...
def build_metrics(records: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "by_feedback": grouped_metrics(records, "feedback"),
    }
...
def save_output(
    ...
    payload = {
        "meta": {
            ...
            "prediction_rule": "feedback contains Human written => human; all other feedback => ai",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run ZeroGPT detectText on human/AI test JSON files and evaluate metrics.")
    parser.add_argument(
        "--dataset",
        action="append",
        default=[],
    ...
    parser.add_argument(
        "-o",
        "--output",
        default="output/zerogpt_detection_eval_output.json",
        help="Single JSON output file for ZeroGPT results and metrics.",
    )
    parser.add_argument("--field", default="text", help="Input JSON field sent to ZeroGPT as input_text")
    parser.add_argument("--endpoint", default=DEFAULT_ENDPOINT, help="ZeroGPT detectText API endpoint")
    ...
    parser.add_argument("--header", action="append", default=[], help="Extra HTTP header, such as 'ApiKey=token'")
...
def main() -> int:
    ...
    print("Request field: input_text")
    print(f"Items to request: {len(jobs)}, resumed: {resumed_count}")
    print("Rule: feedback contains Human written => human; all other feedback => ai")
    ...
    try:
        for batch_number, batch in enumerate(iter_batches(jobs, args.rate_per_second), start=1):
            ...
            batch_results: dict[str, ZeroGPTResult] = {}
            ...
            if args.save_every and processed_count - saved_count >= args.save_every:
                save_output(output_path, endpoint, dataset_specs, args.field, output_items)
